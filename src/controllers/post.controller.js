...
export const getUserPosts = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, posts, `Posts by user ${username}`));
});
...
export const deletePost = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, `Post ${postId} deleted`));
});
...
export const toggleLikePost = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { liked: !liked }, `Toggled like for post ${postId}`)
    );
});
...
export const addComment = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, comment, `Comment added to post ${postId}`));
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        {},
        `Comment ${commentId} deleted from post ${postId}`
      )
    );
});
