...
export const createProblemValidation = [
  ...
  body('timeLimit')
    .optional()
    .isInt({ min: 1 }).withMessage('Time Limit Must be a Positive Number'),

  body('memoryLimit')
    .optional()
    .isInt({ min: 1 }).withMessage('Memory limit must be a positive integer'),
];
...
export const updateProblemValidation = [
  ...
  body("statement")
    .optional()
    .notEmpty().withMessage("Statement cannot be empty"),
  ...
  body("constraints")
    .optional()
    .notEmpty().withMessage("Constraints cannot be empty"),

  body("tags")
    .optional()
    .isArray().withMessage("Tags must be an array")
    .custom((arr) => arr.every((tag) => typeof tag === "string" && tag.trim() !== ""))
    .withMessage("Tags must be an array of non-empty strings"),

  body("timeLimit")
    .optional()
    .isInt({ min: 1 }).withMessage("Time Limit must be a positive integer"),
